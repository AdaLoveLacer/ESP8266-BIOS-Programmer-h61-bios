...
```
     ┌─── 1 CS (Chip Select)
     │ ┌─ 2 DO (MISO)
     │ │ ┌ 3 WP# (Write Protect) → ⚠️ Conectar ao VCC (3.3V)!
     │ │ │┌ 4 GND
  ┌──▼─▼─▼▼─┐
1─┤  ●      ├─8 VCC (3.3V)
2─┤         ├─7 HOLD# → ⚠️ Conectar ao VCC (3.3V)!
3─┤  W25Qxx├─6 CLK
4─┤  Series├─5 DI (MOSI)
  └─────────┘
```

**⚠️ AVISO - CONFIGURAÇÃO CRÍTICA:**
...
- Se WP# estiver em GND, o Status Register fica protegido e você **NÃO CONSEGUIRÁ** remover proteção de bloco!
- Se HOLD# estiver em GND, a comunicação SPI ficará permanentemente pausada!
...
- [ ] CS conectado ao D8 (Pin 1)
- [ ] MOSI conectado ao D7 (Pin 5)
- [ ] MISO conectado ao D6 (Pin 2)
- [ ] CLK conectado ao D5 (Pin 6)
- [ ] **Cabos curtos (máximo 10-15cm)** 📏
...
### 2. Configurar Placa
- **Placa:** "NodeMCU 1.0 (ESP-12E Module)" ou seu modelo específico
- **Upload Speed:** 115200
- **CPU Frequency:** 80 MHz
- **Flash Size:** 4MB (FS:2MB OTA:~1019KB)
- **Porta:** Selecione a porta COM do ESP8266

### 3. Configurar WiFi
Edite as linhas no arquivo `esp8266_w25q32_programmer.ino`:
```cpp
const char* ssid = "SuaRedeWiFi";        // Nome da sua rede
...
### ⚡ Importante: Firmware Tudo-em-Um
**🎯 Basta compilar o arquivo .ino - só isso!**

- ✅ A interface web (HTML/CSS/JS) já está **embutida** no arquivo .ino
- ✅ Não precisa fazer upload de arquivos HTML separados
- ✅ Não precisa usar sistema de arquivos (SPIFFS/LittleFS)
- ✅ Tudo funciona **imediatamente** após a compilação
- 📄 O arquivo `index.html` é apenas para **referência/preview** - não é necessário para operação

**O que acontece:**
1. Você compila `esp8266_w25q32_programmer.ino` no Arduino IDE
2. Faz upload para o ESP8266
3. A interface web **já está lá**, pronta para usar
4. Basta acessar o endereço IP no navegador
...
- **Presets Rápidos:** Botões 1/4/8/20 MHz para testes rápidos
- **Tamanho de Chunk de Verificação:** 4KB, 8KB, 16KB, 32KB, 64KB
- **Recomendação:** 
...
- **Dump Completo:** 
  - ✅ Detecta automaticamente o tamanho do chip
  - ✅ Baixa 2MB-32MB de acordo com o chip conectado
...
- **Barra de Progresso:** Atualizações em tempo real
...
- **Auto-Remover:** Sistema remove proteções automaticamente antes de gravar
- **IMPORTANTE:** Proteção por hardware (pino WP#) deve estar desabilitada (conectada ao VCC)
...
| 0   | 0   | 1   | 256KB superiores | 0x3F0000-0x3FFFFF |
| 0   | 1   | 0   | 512KB superiores | 0x3E0000-0x3FFFFF |
| 0   | 1   | 1   | 1MB superior | 0x3C0000-0x3FFFFF |
| 1   | 0   | 0   | 2MB superiores | 0x380000-0x3FFFFF |
| 1   | 0   | 1   | 3MB superiores | 0x300000-0x3FFFFF |
...
4. Se WP# estiver em GND, remoção **NÃO FUNCIONARÁ**!

## ⚠️ AVISOS CRÍTICOS

### 🚨 BACKUP PRIMEIRO!
**SEMPRE** faça backup da BIOS original antes de qualquer modificação:
...
- Só grave BIOS compatível com sua placa H61
- Nunca desligue durante gravação
- Verifique integridade do arquivo antes de gravar

### ⚡ Precauções Elétricas
- **Desligue completamente** a placa-mãe (cabo de força + bateria)
- Certifique-se que não há energia residual
- Use pulseira antiestática antes de tocar componentes
- Conecte GND primeiro, depois outros pinos
...
**Soluções:**
1. **Atualize para versão mais recente** (correção Base64 implementada)
...
3. **Melhore conexões físicas:**
   ...
   - Verifique fios soltos
   - Confirme terra comum
4. **Verifique logs detalhados:**
   - Console do navegador (F12): tamanho dos chunks enviados
   - Monitor Serial: bytes recebidos/decodificados

### 🔍 ID não detectado ou JEDEC incorreto
- Verifique conexões SPI (ordem dos pinos)
- Confirme alimentação 3.3V estável
...
- **Chip protegido contra escrita:**
  ...
  - **Se WP# estiver em GND, remoção NÃO funcionará!**
- **Chip não responde:**
  - Verifique se **HOLD# (Pin 7)** está conectado a **3.3V** (não GND!)
  - Se HOLD# em GND, chip fica permanentemente pausado
- Voltagem insuficiente (use fonte 3.3V estável)
...
### 💾 Placa-mãe não inicializa após gravação
1. **NÃO ENTRE EM PÂNICO**
2. Re-grave backup original
3. Se backup corrompido, procure BIOS oficial do fabricante
...
|---------|--------|-----------|
| JEDEC ID | 0x9F | Ler ID: EF4016 |
| Read Data | 0x03 | Leitura sequencial |
| Write Enable | 0x06 | Habilitar escrita |
| Page Program | 0x02 | Gravar página (256B) |
| Read Status | 0x05 | Registro de status |
| Sector Erase | 0x20 | Apagar setor (64KB) |
| Chip Erase | 0xC7 | Apagar tudo |
...
│   ├── readJedecId() → Ler ID + detectar capacidade
│   ├── detectFlashSize() → Ajustar FLASH_SIZE dinamicamente
│   ├── readData() → Leitura sequencial
│   ├── programPage() → Gravar 256 bytes
│   ├── sectorErase() → Apagar 4KB
│   └── chipErase() → Apagar tudo
├── 🔒 Sistema de Proteção
│   ├── readStatus() / readStatus2()
│   ├── writeStatusRegister()
│   ├── disableAllProtection() → Remover BP0-BP2
│   └── isWriteProtected() → Verificar proteção
